...
export const config: WebdriverIO.Config = {
    ...
    /* beforeStep: async function (step, context) {
        let stepTxt = (JSON.parse((JSON.stringify(step.text))));
        let btnAcceder = $('//android.widget.TextView[@text="Hazte cliente"]');

        if (stepTxt == 'the PreLogin is displayed') {
            if (!(await (await btnAcceder).isDisplayed())) {
                await browser.reset();
            }
        }
    }, */
